...
> 
> This document details the mathematical models, supply mechanics, and incentive structures governing the $ZPRO token.
...
*   **Token Name:** ZPRO
*   **Symbol:** $ZPRO
*   **Smallest Unit:** Zat (1 $ZPRO = 1,000,000 Zat)
...
Unlike inflationary fiat or fixed-supply assets that rely solely on scarcity, $ZPRO utilizes a **Decaying Emission Curve** (similar to Bitcoin's halving, but adapted for activity-based rewards).
...
*   $R_0$ = Initial Base Reward (e.g., 100 $ZPRO$ per validated activity block)
...
*   **Epoch 0 (Year 1):** 100 $ZPRO$ per valid activity
*   **Epoch 1 (Year 2):** 50 $ZPRO$
*   **Epoch 2 (Year 3):** 25 $ZPRO$
*   **Epoch 3 (Year 4):** 12.5 $ZPRO$
...
*   $A$ = The transaction Amount (in $ZPRO$).
*   $r$ = The fixed fee rate ($r = 0.01$, or **1%**).

**Example:** 
If Player A sends 50 $ZPRO$ to Player B:
*   $Fee = 50 \times 0.01 = 0.5$ $ZPRO$
*   Player B receives: $49.5$ $ZPRO$
*   The $0.5$ $ZPRO$ fee is distributed to the network validators.
...
To prevent infinite inflation and add utility to $ZPRO, certain premium in-game assets (e.g., "Golden Tickets" for extended high-yield gameplay zones) are not minted freely.

### Burn Formula
When a player purchases a premium asset (like a Golden Ticket), they must **permanently burn** a fixed amount of $ZPRO$:
...
*   Price: 5,000 $ZPRO$
*   Transaction: Player sends 5,000 $ZPRO$ to **Null Address** (`WKS-0000000000000000000000000000000000000000`)
*   Result: 5,000 $ZPRO$ are **permanently removed** from circulation
...
*If the sum of rewards in a 24-hour period exceeds the $Daily\_Limit$, all subsequent activities yield $0$ $ZPRO$ until the timer resets.*
...
[ Ledger Updated Locally ] ──> Async Sync to Global Mainnet when Gateway is available
       │
       ▼
[ Premium Purchase ] ──> Burn 5,000 ZPRO to Null Address ──> Supply Decreases