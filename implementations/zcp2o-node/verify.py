...
def _jwk_to_public_key(jwk):
    ...
    return rsa.RSAPublicNumbers(e_int, n_int).public_key(default_backend())

def verify_token(token_b64):
    """
    Verify a ZCP2O Human Proof token (v5).
    ...
    # 6. Verify RSA-PSS signature (byte-exact dengan JSON.stringify JS)
    try:
        ...
        try:
            ...
            # Fallback: format spasi (jaga-jaga)
            pub_key.verify(sig_bytes, json.dumps(payload).encode(),
                padding.PSS(mgf=padding.MGF1(hashes.SHA256()), salt_length=32),
                hashes.SHA256())
    except Exception:
