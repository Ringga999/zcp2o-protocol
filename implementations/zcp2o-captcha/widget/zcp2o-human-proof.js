/* =========================================================
   ZCP2O Human Proof — CORE v0.5.0
   v0.5.0: + Nonce + Expires (anti-replay, P4).
...
(function (global) {
  ...
  function init(opts){
    ...
    async function finish(bonus,bonusLabel){
      ...
      /* ===== P2+P4: NONCE + EXPIRES (anti-replay) ===== */
      const nonceB=new Uint8Array(16);
      crypto.getRandomValues(nonceB);
      const nowS=Math.floor(Date.now()/1000);
      
      const payload={
        v:5,  /* v5 = nonce + expires */
        type:"zcp2o-human-proof",
        challenge:id,
        score:final,
        nonce:b64url(nonceB),  /* 🆕 16 byte random = unique ID */
        layers:{motor:motor.score,sensor:sensor.present?sensor.score:null,[bonusLabel]:bonus},
        signals_digest:await sha256hex(samples),
        issued_at:nowS,
        expires_at:nowS+300,  /* 🆕 5 menit = token mati */
        tier:"light",
        assurance:"self"
      };
      /* ===== END P2+P4 ===== */
